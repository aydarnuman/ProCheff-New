...
class PDFTestRunner {
  ...
  async testPMYODocument(): Promise<any> {
...
İletişim: pmyo@example.gov.tr
  ...
  async testInvalidFile(): Promise<any> {
    const formData = new FormData();
  ...
  async testLargeFile(): Promise<any> {
  ...
  async testEmptyFile(): Promise<any> {
    const formData = new FormData();
  ...
  printReport(report: TestReport): void {
    ...
    if (report.systemHealth?.status) {
      ...
      if (report.systemHealth.recommendations?.length > 0) {
        ...
        report.systemHealth.recommendations.forEach((rec: string) => {
          console.log(`   • ${rec}`);
        });
      }
    }
...
export { PDFTestRunner, TestResult, TestReport };
