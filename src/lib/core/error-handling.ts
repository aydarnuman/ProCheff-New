...
export interface FallbackStrategy<T> {
  execute(error: ProCheffError, originalInput: unknown): Promise<T>;
...
export class ErrorHandler {
  private fallbackStrategies: Map<ErrorCode, FallbackStrategy<unknown>[]> =
    new Map();
  ...
  async handleWithFallback<T>(
    ...
    try {
      ...
      const procheffError =
        error instanceof ProCheffError
          ? error
          : new ProCheffError({
              code: errorCode,
      ...
      for (const strategy of strategies) {
        if (strategy.canHandle(procheffError)) {
          try {
            return (await strategy.execute(procheffError, originalInput)) as T;
          } catch (fallbackError) {
