...
export class ObservabilityManager {
  ...
  startTrace(operation: string, input: unknown): string {
  ...
  endTrace(traceId: string, output: unknown, success: boolean) {
    const trace = this.traces.get(traceId);
    if (!trace) return;
  ...
  async collectSystemMetrics(): Promise<SystemMetrics> {
    ...
    const metrics: SystemMetrics = {
      ...
      marketData: (await this.collectMarketDataMetrics()) as any,
      system: {
  ...
  getSystemHealth(): {
    ...
    // Check API endpoints
    Object.entries(latestMetrics.apiEndpoints).forEach(
      ([endpoint, metrics]) => {
        ...
        if (metrics.responseTime > 5000) {
          issues.push(
            `Slow response time on ${endpoint}: ${metrics.responseTime}ms`
          );
          status = "degraded";
        }
      }
    ...
    if (latestMetrics.system.memoryUsage > 500) {
      // 500MB threshold
      issues.push(
        `High memory usage: ${latestMetrics.system.memoryUsage.toFixed(1)}MB`
      );
      status = "degraded";
    }
...
export async function recordMetric(
  ...
  try {
    ...
    console.warn("observability.recordMetric.failed", {
      name,
...
export async function recordOcrMetrics(params: {
  docHash?: string;
  endpoint?: string;
  method?: string; // pdf-parse | ocr | hybrid
  ...
  const labels = {
    endpoint,
    method,
    earlyStop: earlyStop || "none",
    extractionChain: (extractionChain || []).join(","),
    docHash,
  } as Record<string, unknown>;

  // Limit writes to a few lines per run
  await recordMetric("ocr_avg_confidence", avgConfidence ?? 0, labels);
  ...
  if (typeof processingMs === "number") {
    await recordMetric("ocr_processing_ms", processingMs, labels);
  }
}
...
export function logDataTransformation(
  ...
  observability.addTransformation(
    traceId,
    operation,
    input,
    output,
