 ...
 */
export function quickReasoning(
  menu: MenuAnalysis,
  offer: OfferResult
): {
  score: number;
...
export function generateDetailedReport(
  ...
  // Detaylı skorlama
  const nutritionRisks = reasoning.risks.filter(
    (r) =>
      r.includes("protein") ||
      r.includes("karbonhidrat") ||
      r.includes("yağ") ||
      r.includes("çeşitlilik")
  );
  const financialRisks = reasoning.risks.filter(
    (r) => r.includes("kâr") || r.includes("kar") || r.includes("maliyet")
  );
  const complianceIssues = reasoning.compliance.filter(
    (c) => c.includes("UYARI") || c.includes("kuralı")
  );
  ...
  const immediateActions = reasoning.suggestions.filter(
    (s) => s.includes("protein") || s.includes("kâr") || s.includes("eşik")
  );
  const mediumTerm = reasoning.suggestions.filter((s) => !immediateActions.includes(s));
  ...
  return {
    ...
    risk_analysis: {
      ...
      critical_issues: reasoning.risks.filter(
        (r) => r.includes("düşük") || r.includes("yüksek") || r.includes("yetersiz")
      ),
    },
    recommendations: {
